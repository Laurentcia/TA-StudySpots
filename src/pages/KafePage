...
const KafePage = ({ spots, onSpotClick, favorites }) => {
  const [searchQuery, setSearchQuery] = useState('');
  
  const filteredSpots = spots.filter(spot => {
      return spot.category === 'kafe' && spot.name.toLowerCase().includes(searchQuery.toLowerCase());
  });
  ...
  return (
    <div className="min-h-screen bg-[#F8EDED] pb-24 md:pb-12 pt-10">
      <main className="max-w-7xl mx-auto px-6">
        <div className="mb-10">
          <h1 className="text-4xl md:text-6xl font-black text-[#1F3F49] tracking-tighter mb-2">Work & Coffee<span className="text-[#D25045]">.</span></h1>
          <p className="text-[#1F3F49]/60 font-medium">Explore the best spots in town.</p>
        </div>
